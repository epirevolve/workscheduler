...
def login():
    if request.method == 'POST':
        def login_check(x: Operator) -> bool:
            return request.form['username'] == x.name and request.form['password'] == x.password
        user = next(filter(login_check, user_repository.get_users()), None)
        if not user:
...
def show_operators():
    return render_template('operators.html', users=user_repository.get_users())
