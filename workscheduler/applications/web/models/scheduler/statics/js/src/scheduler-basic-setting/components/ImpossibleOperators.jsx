...
const impossibleOperators = ({ workCategory, onImpossibleOperatorChange }) => {
    ...
    const impossibleOperators = workCategory.impossibleOperators.map(x =>
        <ListItem key={x.id}>
            <ListItemText primary={x.user.name} />
        </ListItem>);
        ...
        <React.Fragment>
            ...
            <ExpansionPanel expanded={expanded} onChange={onExpandedChange}>
                <ExpansionPanelSummary expandIcon={<ExpandMoreIcon />}>
                    <Typography variant="h6" style={{ color: 'gray' }} className="mt-2">impossible operators</Typography>
                    <div className="ml-3">
...
export default impossibleOperators;