...
const weekDayOperators = ({ workCategory, onWeekDayOperatorChange }) => {
    const weekDayOperatorIds = workCategory.weekDayOperators.map(x => x.id);
    const operatorList = operators.map(x =>
        <ListItem key={x.id} button onClick={() => onWeekDayOperatorChange(workCategory.id, x)}>
            <Checkbox checked={weekDayOperatorIds.includes(x.id)} tabIndex={-1} disableRipple />
            <ListItemText primary={x.user.name} />
        </ListItem>);
    const weekDayOperators = workCategory.weekDayOperators.map(x =>
        <ListItem key={x.id}>
            <ListItemText primary={x.user.name} />
        </ListItem>);
        ...
        <React.Fragment>
            ...
            <ExpansionPanel expanded={expanded} onChange={onExpandedChange}>
                <ExpansionPanelSummary expandIcon={<ExpandMoreIcon />}>
                    <Typography variant="h6" style={{ color: 'gray' }} className="mt-2">week day operators</Typography>
                    <div className="ml-3">
...
export default weekDayOperators;