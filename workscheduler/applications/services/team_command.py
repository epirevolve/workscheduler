...
from workscheduler.domains.models.team import (
    TeamCategoryFactory, TeamFactory
)
...
class TeamCommand:
    ...
    def append_team_category(self, id: str, name: str, is_permit_multi_belonging: bool, is_leader_required: bool, min_member_count: int, max_member_count: int):
        team_category = TeamCategoryFactory.register_a_team_category(id, name, is_permit_multi_belonging, is_leader_required, min_member_count, max_member_count)
        self._session.add(team_category)

    def update_team_category(self, id: str, name: str, is_permit_multi_belonging: bool, is_leader_required: bool, min_member_count: int, max_member_count: int):
        team_category = TeamQuery(self._session).get_team_category(id)
        team_category.name = name
        team_category.is_permit_multi_belonging = is_permit_multi_belonging
    ...
    def append_team(self, team_category_id: str, name: str, all_member_info):
        team = TeamFactory.register_a_team(name, team_category_id)
        for member_info in all_member_info:
            if member_info[1]:
                team.leader_user_id = member_info[0]
            team.users.append(UserQuery(self._session).get_user(member_info[0]))
